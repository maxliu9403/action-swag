...
3. 在包含`main.go`文件的项目根目录运行`action-swag init`。这将会解析注释并生成需要的文件（`docs`文件夹和`docs/docs.go`）。
...
4. (可选) 使用`action-swag fmt`格式化 SWAG 注释。(请先升级到最新版本)
...
1. 使用`action-swag init`生成Swagger2.0文档后，导入如下代码包：
