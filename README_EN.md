...
3. Run `action-swag init` in the project's root folder which contains the `main.go` file. This will parse your comments and generate the required files (`docs` folder and `docs/docs.go`).
```sh
$ action-swag init
```
...
4. (optional) Use `action-swag fmt` format the SWAG comment. (Please upgrade to the latest version)
...
1. After using `action-swag init` to generate Swagger 2.0 docs, import the following packages:
```go
